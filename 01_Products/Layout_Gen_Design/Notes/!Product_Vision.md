...
- [ ] Expand from 3 → **12 groups**: Power Block, Cooling Tower, Admin Building, Gate House, Cable Tunnel, LPG/Metering, Flare, WT/WWT, Water + 3 polyline racks (Pipe Rack, Main Rack, Utility Rack).
- [ ] **Generic Rule Engine:** Replace 6 hand-coded rule functions with a `RULES` data list + generic evaluators dispatched by **Rule Type** (`min_distance`, `max_distance`, `center_proximity`, `boundary_setback`, `windward_edge`, `pipe_rack_proximity`).
- [ ] Add **Rule Type** column to `!Scoring_Logic.md` — each row maps 1:1 to a dict in the `RULES` list in `Rules.py`.
- [ ] **Polyline Racks:** Pipe Rack, Main Rack, Utility Rack rendered as straight lines (not rectangles).
- [ ] **Rule Network Visualizer:** Standalone `Notes/Rule_Network.html` using **Pyvis** — interactive physics-based graph showing all groups (nodes) and rules (edges). Generated by `Core/RuleNetwork.py`.
- [ ] Update generative engine to place all 12 groups.

### Phase_05: Generative Optimization & Sub-Clusters

- [ ] Deconstruct groups into individual sub-buildings (60+ items from Excel).
- [ ] Advanced scoring engine with weighted rule priorities.
- [ ] Cross-group sub-building dependencies (node network architecture).

## 📍 Current Status: **Phase_04 (New Groups & Generic Rules)**
