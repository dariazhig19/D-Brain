...
| `road_proximity`      | `_eval_road_proximity()`    | 건물/랙에서 가장 가까운 도로까지의 거리             | Linear       |
| `boundary_overflow`   | `_eval_boundary_overflow()` | 건물이 부지 경계를 넘었을 때의 페널티              | Logarithmic  |
...
| **GIS-01** | GIS                | `boundary_setback` | 주 도로           | 15 m  | 1000 pts (flat) | 부지 경계로부터 이격          |
| **WH-01** | Warehouse          | `boundary_setback` | 주 도로           | 15 m  | 1000 pts (flat) | 부지 경계로부터 이격          |
| **LP-01** | LPG/Metering       | `boundary_setback` | 주 도로           | 10 m  | 1000 pts (flat) | 부지 경계로부터 이격          |
...
| **WW-01** | WT/WWT             | `boundary_setback` | 주 도로           | 10 m  | 1000 pts (flat) | 부지 경계로부터 이격          |
| **WW-02** | WT/WWT             | `leeward_edge`     | 풍향             | 50 m  | 500 pts (flat)  | 반드시 **풍하측**에 있어야 함 |
| **WA-01** | Water              | `boundary_setback` | 주 도로           | 10 m  | 1000 pts (flat) | 부지 경계로부터 이격          |
...
| ID        | 그룹             | 규칙 유형              | 대상             | 임계값  | 페널티             | 조건                 |
| :-------- | :------------- | :----------------- | :------------- | :--- | :-------------- | :----------------- |
| **RD-01** | Power Block    | `road_proximity`    | Perimeter Road | 3 m  | 500 pts / m     | 건물과 도로 사이의 간격      |
| **RD-02** | Cooling Tower  | `road_proximity`    | Perimeter Road | 3 m  | 500 pts / m     | 건물과 도로 사이의 간격      |
| **RD-03** | Admin Building | `road_proximity`    | Perimeter Road | 3 m  | 500 pts / m     | 건물과 도로 사이의 간격      |
...
| **Cable Tunnel** | 3 m | 지하 케이블 경로                 | GIS ↔ Power Block                                                             |
