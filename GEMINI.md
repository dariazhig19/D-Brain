...
## Workflow Mandates
- **Input Processing**: When analyzing or moving files from `00_Input`, strictly follow the Distribution Rules (01_Products, 02_Library, or 05_Reports).
- **Product Structure**: Maintain the mandatory subfolder structure for each product: `Core/`, `Dashboard/`, `Data/`, `Notes/`, and `Pipeline/`.
- **Reporting**: Every workday/session must conclude with a `Daily_Report_YYYYMMDD.md` in `04_Reports/Daily/` (Note: `!Atlas_System.md` mentions `05_Reports`, but the current directory structure shows `04_Reports`).
